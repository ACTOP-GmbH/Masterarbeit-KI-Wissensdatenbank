...
Auch im Vergleich zu tatsächlichen KI-Servern ist der Preisvergleich vorteilhaft.
Eine beispielhafte Grafikkarte wie die RTX 6000 Ada mit 48GB VRAM kostet im Einzelhandel derzeit mindestens 7000€ \cite{rtx6000_price}, während komplette Serverlösungen mit mehreren GPUs schnell in den fünfstelligen Bereich gelangen oder gar lediglich leistungsstarke Grafikkarten wie die H100 für über 30.000€ angeboten werden.
Diese Preisdifferenz macht den Einsatz von handelsüblicher Hardware für kleine und mittlere Unternehmen besonders attraktiv, da sie eine kosteneffiziente Alternative zu teuren Serverlösungen darstellt.
Für viele Positionen in vielen Unternehmen ist dies eine Investition, welche ein ganzes Jahresgehalt übersteigen kann.
Es muss sich also rentieren, eine solche Hardware anzuschaffen, anstatt auf Cloud-Dienste zurückzugreifen oder bei analoger, menschlicher Recherche zu verbleiben.
...
Gemäß der in Kapitel~\ref{subsec:zielarchitektur-modularitaet-entkopplung} beschriebenen Zielarchitektur fiel die Wahl auf die Open-Source-Plattform \textit{Onyx} (ehemals \textit{Danswer}) als technisches Fundament.
...
Onyx zerlegt Dokumente standardmäßig in Chunks anhand eines Größenlimits (token-/zeichenbasiert) mit moderater Überlappung.
Die Chunk-Grenzen orientieren sich dabei nicht zwingend an semantischen Dokumentstrukturen wie Überschriften; dadurch können zusammengehörige Inhalte (z. B. Frage/Antwort-Paare) getrennt werden.
Exakte Werte hierzu sind in der Onyx-Dokumentation nicht spezifiziert.

Der Ablauf sieht in Onyx insgesamt so aus:

\vspace{1cm}
\FloatBarrier
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Bilder/onyx embedding}
    \caption{Chunking-Prozess in Onyx~\cite{onyx_data_flows})}
    \label{fig:onyx_chunking}
\end{figure}
\vspace{1cm}
\FloatBarrier
...
Der fertige Prozess sieht damit folgendermaßen aus:
\vspace{1cm}
\FloatBarrier
\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{Bilder/onyx query}
    \caption{RAG-Anfrageprozess in Onyx~\cite{onyx_data_flows})}
    \label{fig:onyx_query}
\end{figure}
\vspace{1cm}
\FloatBarrier
...
Vor der eigentlichen Evaluation wurde der RAG-Chatbot zunächst gemäß der in Kapitel~\ref{subsec:notwendige-architekturkomponenten} beschriebenen Architektur implementiert und in einer ersten lauffähigen Konfiguration in Betrieb genommen.
...
\newglossaryentry{litellm}{
    name=litellm,
    description={Eine leichtgewichtige Python-Bibliothek zur Kommunikation mit lokalen LLM-Diensten über HTTP. Wird von Onyx zur Anbindung von Ollama genutzt.}
}
...
Erst eine detaillierte Analyse der Log-Dateien und der litellm-Konfiguration machte deutlich, dass hier kein fehlerhaftes Modell vorlag, sondern eine unpassende Timeout-Konfiguration, die schlicht nicht zu den Laufzeiten komplexer LLM-Inferenz passte.
...
Da aber eine Anpassung der Konfigurationsdatei von Docker ignoriert wurde, wurde im Quellcode von Onyx eine feste Änderung vorgenommen, um die litellm-Timeouts dauerhaft zu erhöhen.

Die Diagnose des Timeout-Problems lässt sich auf Ebene des Onyx-Quellcodes präzisieren.
Onyx kapselt die Kommunikation mit externen Sprachmodellen in einer eigenen Wrapper-Klasse \texttt{DefaultMultiLLM}, über die sämtliche Aufrufe an \texttt{litellm} geroutet werden.
In dieser Klasse wird festgelegt, mit welchem Zeitlimit (\emph{Timeout}) Anfragen an \texttt{litellm} abgesetzt werden.
Die für diese Arbeit relevante Implementierung befindet sich in \texttt{backend/onyx/llm/chat\_llm.py} und wird folgend dargestellt.
...
class DefaultMultiLLM(LLM):
    def __init__(
        ...
        # weitere Initialisierung (Temperatur, Provider, kwargs, ...)
        ...
    ...
    def _completion(
        ...
        try:
            return litellm.completion(
                ...
                model=f"{self.config.model_provider}/"
                ...
                timeout=self._timeout,  # <- hier greift der konfigurierte Wert
                **self._model_kwargs,
            )
        except Exception as e:
            ...
    \end{minted}
    \caption{Vereinfachter Ausschnitt aus \texttt{DefaultMultiLLM} in Onyx}
    \label{lst:defaultmultillm-timeout}
\end{listing}
\FloatBarrier

Konzeptionell war damit vorgesehen, das für Frage-Antwort-Szenarien relevante Zeitlimit zentral über den Konfigurationsparameter \texttt{QA\_TIMEOUT} zu steuern, der aus der \texttt{.env}-Datei in \texttt{onyx.configs.chat\_configs} eingelesen wird.
In der Praxis zeigte sich jedoch, dass die Konfiguration im konkreten Docker-Setup nicht wie beabsichtigt durchschlug:
Trotz Anpassung von \texttt{QA\_TIMEOUT} in der \texttt{.env}-Datei und anschließendem Neustart der Container änderte sich das tatsächlich wirksame Zeitverhalten nicht.
Anfragen mit umfangreichem RAG-Kontext brachen weiterhin nach wenigen Sekunden ab.
Log-Ausgaben und reproduzierbare Tests mit identischen Prompt-Konstellationen machten deutlich, dass die eigentliche Ursache nicht in der Python-Logik von \texttt{DefaultMultiLLM}, sondern im Zusammenspiel von Container-Konfiguration, Umgebungsvariablen und der Art lag, wie Onyx bzw.\ \texttt{litellm} die Defaults für HTTP-Timeouts intern handhaben.

Vor dem Hintergrund des Projektziels – eine stabile, für die Evaluation geeignete Instanz – wurde daher auf eine tiefgreifende Generalüberholung der Konfigurations- und Settings-Schicht verzichtet.
Stattdessen wurde eine bewusst pragmatische Lösung gewählt: Das Timeout wurde im Onyx-Wrapper explizit auf einen ausreichend hohen Wert fest verdrahtet und zusätzlich per Logging verifiziert.
In der für diese Arbeit verwendeten Fassung lautet der relevante Ausschnitt daher wie in Listing~\ref{lst:defaultmultillm-hardcoded}.

\FloatBarrier
\begin{listing}[H]
    \begin{minted}[fontsize=\footnotesize, linenos]{python}
        class DefaultMultiLLM(LLM):
        def __init__(..., timeout: int | None = None, ...):

        self._timeout = 600
        if timeout is None:
        if model_is_reasoning_model(model_name, model_provider):
        self._timeout = 600 * 10  # Reasoning-Modelle: längeres Timeout
        else:
        self._timeout = 600
        ...
        ...
        try:
        logger.error(f"CALLING LITELLM with timeout={self._timeout}")
        return litellm.completion(
        ...,
    ...
    \end{minted}
    \caption{Vereinfachter Ausschnitt aus \texttt{DefaultMultiLLM} in Onyx}
    \label{lst:defaultmultillm-hardcoded}
\end{listing}
\FloatBarrier

Damit wird die ursprünglich vorgesehene, konfigurationsgetriebene Steuerung über \texttt{QA\_TIMEOUT} bewusst umgangen.
Das Zeitlimit ist nun vollständig im Anwendungscode definiert und unabhängig von der Frage, ob die entsprechenden Umgebungsvariablen im konkreten Container-Setup korrekt propagiert werden.
Dies reduziert zwar die Flexibilität aus Sicht einer generischen Onyx-Distribution, erhöht im Gegenzug aber die Betriebssicherheit der konkreten Instanz:
Für die Zwecke dieser Masterarbeit ist entscheidend, dass komplexe Anfragen mit umfangreichem Kontext und aktivierter Contextual-RAG-Pipeline nicht durch zu aggressive Standard-Timeouts von \texttt{litellm} oder dem darunterliegenden HTTP-Client abgebrochen werden.

Aus methodischer Sicht ist diese Entscheidung ein bewusst eingegangener Kompromiss:
Anstatt die gesamte Konfigurationskette aus \texttt{.env}, Pydantic-Settings und Docker-Orchestrierung zu refaktorisieren und damit zahlreiche potenzielle Fehlerquellen und zukünftige Wartungsaufwände (etwa bei Upstream-Updates von Onyx) zu eröffnen, wurde ein eng umgrenzter, gut nachvollziehbarer Eingriff an einer zentralen Stelle vorgenommen.
Die dadurch erreichte Stabilität ist Voraussetzung für die in Kapitel~\ref{sec:evaluation} beschriebenen Experimente, in denen die Wirkung unterschiedlicher Modelle, RAG-Konfigurationen und Prompt-Strategien systematisch untersucht wird.

Nachdem auch ein Wechsel zum neuen gpt-oss20b Modell kein zufriedenstellendes Gleichgewicht zwischen Antwortzeit und Antwortqualität erreicht werden konnte, wurde im nächsten Schritt ein kleineres, explizit für Frage-Antwort-Szenarien beworbenes Modell gewählt.
