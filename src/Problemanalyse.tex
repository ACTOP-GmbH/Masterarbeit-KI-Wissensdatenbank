...
\subsection{Ist-Zustand und Problemstellung}\label{subsec:ist-zustand-und-problemstellung}
...
Daraus resultiert die Gefahr, dass ein LLM auf spezifische Fragen entweder gar keine oder fehlerhafte Antworten (sog.\ \textit{Halluzinationen}) gibt~\cite{wikipedia_rag}.
...
Vertrauliche Unternehmensdaten dürfen nicht unkontrolliert an externe Dienste gesendet werden~\cite{punctuations_onpremise_llm}.
...
Zusätzlich fehlt es generischen LLMs an Aktualität und Domänenwissen: Sie könnten halluzinierende Antworten geben, also sachlich falsche Auskünfte erteilen, wenn die Anfrage sich auf interne Details bezieht~\cite{wikipedia_rag_improvement}.
...
\begin{enumerate}
    ...
    So kann es beispielsweise passieren, dass \textit{automatisierte Zugriffe} von Sicherheitsmechanismen blockiert werden~\cite{airom2024rag_enterprise}.
    ...
    Insbesondere müssen bestehende Berechtigungskonzepte aus Systemen wie SharePoint respektiert werden, sodass der Chatbot einem Nutzer nur solche Informationen liefert, die dieser auch regulär lesen darf~\cite{realmlabs_secure_rag}.
...
\subsection{Stakeholder und Rollen}\label{subsec:stakeholder}
...
\vspace{1cm}
\FloatBarrier
\subsubsection{Use Cases}\label{subsubsec:use-cases}
...
\begin{table}[htbp]
    \centering
    \small
    \begin{tabular}{p{1.2cm}p{3.4cm}p{3cm}p{5cm}p{1.6cm}}
        \toprule
...
\paragraph{Verdichtung}
Aus Stakeholdern, User Stories und Use Cases ergeben sich als zentrale Anforderungen eine schnelle, belegbare Fragebeantwortung auf Basis interner Dokumente, ein strikt berechtigungsbewusstes Retrieval sowie ein on-premises Betrieb mit administrierbarer Wissensbasis und nachvollziehbarem Logging.
Damit verschiebt sich der Fokus von klassischer Dokumentensuche hin zu einer integrierten Wissensschnittstelle, die Retrieval und Generierung kontrolliert kombiniert.


\subsection{Soll-Zustand und Qualitätsziele}\label{subsec:soll-zustand}
...
\subsection{Anforderungen und Randbedingungen}\label{subsec:anforderungen}
...
\subsubsection{CPU vs GPU}\label{subsubsec:cpu-vs-gpu}
...
\begin{itemize}
    ...
    \item \textbf{Arbeitsspeicher:} 16 GB DDR4 RAM
...
\subsection{Toolauswahl und Begründung}\label{subsec:toolauswahl}
Auf Basis der Anforderungen und Randbedingungen wurden potenzielle Open-Source-Tools evaluiert und anhand eines gewichteten Kriterienmodells bewertet.

\subsubsection{Open-Source Tools und Frameworks}\label{subsubsec:open-source-tools-und-frameworks}
...
\begin{table}[htbp]
    \centering
    \small
    \begin{tabular}{lp{1.5cm}p{2.4cm}p{2.2cm}p{1.1cm}p{2.6cm}}
        \toprule
...
\section{Konzept und Zielarchitektur}\label{sec:architektur}
...
\subsection{Zielarchitektur zu Modularität und Entkopplung}\label{subsec:zielarchitektur-modularitaet-entkopplung}
...
\subsection{Rolle des Web-Interfaces als Orchestrierungsschicht}\label{subsec:rolle-des-web-interfaces-als-orchestrierungsschicht}
...
\subsection{Skalierbarkeit und Betriebsmodelle: modularer Monolith vs.\ Microservices}\label{subsec:skalierbarkeit-und-betriebsmodelle:-modularer-monolith-vs.-microservices}
...
\subsection{Optionale Erweiterung der Kommunikationskanäle}\label{subsec:optionale-erweiterung-der-kommunikationskanale}
